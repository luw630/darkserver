...
char AstarPathFind::GetFromOpenQueue()
{
    ...
    if(m_Close != NULL)
    {
        for(iter = m_FullOpenList.begin();iter != m_FullOpenList.end();iter++)
        {
            Nod Tile;
            Tile = (*iter);
            if(m_Close->Pos == tile_pos(Tile.x,Tile.y))
            {
                m_FullOpenList.erase(iter);
                break;
            }
        }
    }
...
{
    ASHORT ret;
    ret = (ASHORT)sqrt( (float)((xstart-xend)*(xstart-xend) + (ystart-yend)*(ystart-yend)) );
    return ret;
}
...
{
    if ( MoveAble == NULL || JudgeCost == NULL )
        return -1;

    TAstarNode *root;
    TAstarNode *pBestNode;
    ...
    //如果起点不可走
    if((!MoveAble(startx,starty )))
        return -1;
    if(startx > m_width || endx > m_width || starty > m_height || endy > m_height)
        return -1;
...
//int AstarPathFind::GetPathInfoCount(void)
//{
//	short count = 0;
...
int AstarPathFind::Create(int mapw,int maph,char (*MoveCheck)(ASHORT,ASHORT),ASHORT (*JudgeFun)(ASHORT x,ASHORT y,ASHORT xend,ASHORT yend))
{
	int i,j;
    m_SmoothCount = 0;
	...
	SetDirMask(0xFF);
    //设置open表的最大值
	SetOpenLimited(m_MaxSize);
    //设置close表的最大值
	SetCloseLimited(m_MaxSize);
#ifdef _MYDEBUG
    m_FullOpenList.clear();
#endif
	return 0;
...
void AstarPathFind::SetMapCheck(char (*MapCheck)(ASHORT,ASHORT)) { MoveAble=MapCheck; }
