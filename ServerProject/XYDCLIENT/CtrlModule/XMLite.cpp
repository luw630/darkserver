...
LPTSTR _tagXMLNode::Load( LPCTSTR pszXml, LPPARSEINFO pi /*= &piDefault*/ )
{
    ...
    if( _tcsncmp( xml, "<!--", 4 ) == 0 )
    {
        TCHAR* pCommentEnd = _tcsstr( xml, "-->" );
        return pCommentEnd + 2;
    }

    // Ignore version info
    if( _tcsncmp( xml, "<?xml", 5 ) == 0 )
    {
        TCHAR* pVersionEnd = _tcsstr( xml, "?>" );
        return Load( pVersionEnd + 1, pi );
    }
