...
//	int	  m_nBkColor;
	...
	bool m_bMultiline;
	size_t m_nShowCur;   // 显示偏移量
    std::list<wchar_t> bufex;
    std::list<wchar_t>::iterator it;

public:
	CDEdit(int fontsize = 16);
	~CDEdit(void);  
...
//	void			SetBkColor(int nClr) { m_nBkColor = nClr;};                 // 设置背景颜色
	...
	void			AddChar(int nChar);                                         // 添加字符
	void			AddCharEx(int nChar);                                         // 添加字符
	...
	void			GetStringEx( char *pBuff, long size );                        // 返回输入的字符串
	...
	void			SetMultiline(bool bMultiline = true){m_bMultiline = bMultiline;};// 设置编辑筐 多行/false单行
    size_t          GetMaxLen(){return m_nMaxLen;};
