...
int InitCommandMap()
{
	...
	command_map["onsetdata"] = 511;
	command_map["myclass"] = 512;
	return 1;
}
...
// 			}
	...
	case 511: //读取工会
	{
		lua_getglobal(g_Script.ls, "OnSetData");
		//lua_pushinteger(g_Script.ls, 0);
		BYTE *buffer = reinterpret_cast <BYTE*>(lua_newuserdata(g_Script.ls, 0xf000));
		if (buffer)
		{
			memset(buffer, 0, 0xf000);

			DWORD64  tick = GetTickCount64();
			memcpy(buffer, &tick, sizeof(DWORD64));
		}

		//lua_rawseti(g_Script.ls, -2, 1);
		if (lua_pcall(g_Script.ls, 1, 0, 0) != 0)
		{
			char err[1024];
			sprintf(err, "%s\r\n", lua_tostring(g_Script.ls, -1));
			rfalse(2, 1, err);
			lua_settop(g_Script.ls, 0);
			return;
		}
	}
	break;
