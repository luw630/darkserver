...
BOOL CPlayerManager::OnCheckAccountServer(DNID dnidClient, BOOL bCheckWhiteList,  SACheckWhiteList    *pMsg)
{
	...
	SACheckAccountMsg samsg;
	samsg.byResult = SACheckAccountMsg::RET_SUCCESS;
	samsg.dnidClient = pMsg->dnidPlayerClient;
	samsg.wGMLevel = 0;
	memcpy(samsg.streamData, pMsg->streamData, MAX_ACCAPASS);
	GetApp()->m_AccountClient.RecvCheckAccountMsg(&samsg);
// 	try
// 	{
// 		lite::Serialreader sl(pMsg->streamData);
// 		LPCSTR	account = sl();
...
// 	   LPCSTR abs_account = Globals::SplitAccount(account, prefix);
// 
// 		lite::Serializer slm(samsg.streamData, sizeof(samsg.streamData));
// 		slm(abs_account)(password);
// 		slm(macAddr);
// 
// 		samsg.byResult = SACheckAccountMsg::RET_SUCCESS;
