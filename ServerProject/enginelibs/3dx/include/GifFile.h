...
// joexo addon
struct ExtensionGraphicControler
{
    BYTE _x21;
    BYTE _xf9;
    BYTE _size_cb4;
...
private:
    ExtensionGraphicControler *curGControler;
...
public:
	GifFile();
	BOOL		Open(LPSTR lpFileName, Bitmap* lpBitmap);
	BOOL		Save(LPSTR lpFileName, Bitmap* lpBitmap);
	BOOL		FirstImage();
	BOOL		NextImage();

    // joexo additional
	BOOL		LoadToBitmap( Bitmap &bitmap, LPVOID buffer, size_t size );     // 直接通过一个内存块初始化对象，并获取第一桢数据
    DWORD       CurFrameInterval();                                             // 在动画序列进行过程中，获取当前桢所需要停留的时间（如果只有一桢，则返回值为-1）
    BOOL        Eof();                                                          // 当前桢是否为动画序列中的最后一桢！
};
//------------------------------------------------------------------------------
#endif
