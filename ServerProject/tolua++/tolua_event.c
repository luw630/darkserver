...
static void storeatubox (lua_State* L, int lo)
{
	...
	lua_getuservalue(L, lo);	/* stack: k,v,table */
#else
	lua_getfenv(L, lo);                       /* stack: mt newud */
#endif
...
#else
		lua_setfenv(L, lo);                         /* stack: mt newud */
#endif
...
#if LUA_VERSION_NUM > 501
		lua_getuservalue(L, 1);	/* stack: k,v,table */
#else
		lua_getfenv(L, 1);                       /* stack: mt newud */
#endif
        if (!lua_rawequal(L, -1, TOLUA_NOPEER)) {
            lua_pushvalue(L, 2); /* key */
